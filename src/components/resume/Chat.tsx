    ...
    <div className="h-screen relative min-w-1/2">
        <div className="w-full ">
            <div className=" flex flex-col justify-end">
                {
                    state.chat.map((message, index)=>{
                        return (<p key = {index} className={`  w-full  p-4 px-3 rounded-md shadow-sm ${index % 2 === 0 && "bg-[#F4F7FF] self-end"}`}>
                            {
                                message
                            }
                            </p>)
            ...
            <div className="w-full flex fixed z-20 bottom-1 rounded-r-md ">
                <input onChange={(e) => setmessage(e.target.value)} value={message}  type='text' className="w-[42%] z-20 p-5 border-2 rounded-l-md "  />
                <button onClick={submitHandler} className=" w-[8%] flex justify-center bg-primary items-center rounded-r-md"> <BsSend color="white" size={22}/></button>
            </div>
